...
export default function BecomeProviderScreen() {
  ...
  const handleWhatsAppContact = (customMessage?: string) => {
    ...
    const defaultMsg = "Olá! Gostaria de saber mais sobre como me tornar um prestador no XamaJá.";
  ...
  const isPending = provider && (provider.status === "pendente" || !provider.isActive);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        >
          <MaterialIcons name="arrow-back" size={20} color="#6B7280" style={{ marginRight: 4 }} />
      ...
      {isPending ? (
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          <View style={styles.heroSection}>
            <View style={[styles.heroIcon, { backgroundColor: "#FFFBEB", borderColor: "#FDE68A" }]}>
              <MaterialIcons name="watch-later" size={40} color="#F59E0B" />
            </View>
            <Text style={styles.heroTitle}>Aguardando Aprovação</Text>
            <Text style={styles.heroSubtitle}>
              Seu cadastro foi realizado com sucesso! O XamaJá realiza uma análise detalhada dos perfis para garantir a qualidade de nossos serviços.
            </Text>
          </View>
          ...
          <View style={styles.contactInfoCard}>
            ...
            <Text style={styles.contactText}>
              A aprovação do seu perfil pode levar até 24 horas úteis. Para agilizar o processo e obter ativação imediata, clique no botão abaixo para conversar diretamente conosco pelo WhatsApp.
            </Text>
          </View>
          ...
          >
            ...
            <Text style={styles.whatsappBtnText}>Solicitar Ativação Rápida</Text>
          </Pressable>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          ...
          <View style={{ paddingHorizontal: 16, marginTop: 24, gap: 12 }}>
            <Pressable
              style={({ pressed }) => [styles.registerBtnAction, pressed && { opacity: 0.85 }]}
              onPress={() => router.push("/register-professional" as any)}
            >
              <MaterialIcons name="assignment" size={22} color="#fff" />
              <Text style={styles.registerBtnActionText}>Criar Perfil no Aplicativo</Text>
            </Pressable>

            <Pressable
              style={({ pressed }) => [styles.secondaryContactBtn, pressed && { opacity: 0.85 }]}
              onPress={() => handleWhatsAppContact()}
            >
              <MaterialIcons name="chat" size={20} color="#25D366" />
              <Text style={styles.secondaryContactBtnText}>Falar com a Central (WhatsApp)</Text>
            </Pressable>
          </View>
...
const styles = StyleSheet.create({
  ...
  secondaryContactBtnText: { fontSize: 14, fontWeight: "700", color: "#25D366" },
  ...
  infoCardTitle: { fontSize: 16, fontWeight: "700", color: "#111827", marginBottom: 4 },
  infoCardRow: { flexDirection: "row", justifyContent: "space-between", borderBottomWidth: 1, borderBottomColor: "#F3F4F6", paddingBottom: 6 },
  infoCardLabel: { fontSize: 14, color: "#6B7280", fontWeight: "600" },
  infoCardValue: { fontSize: 14, color: "#111827", fontWeight: "700", textAlign: "right", flex: 1, marginLeft: 16 },
