...
export const favoritesRouter = router({
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    return dbFavorites.map((pro) => ({
      ...
      avatar:
        pro.avatarUri ||
        `https://ui-avatars.com/api/?name=${encodeURIComponent(pro.name)}`,
      rating: Number(pro.rating) || 0,
      phone: pro.phone || pro.whatsapp || "",
      type: (typeof pro.plan === "string" &&
      (pro.plan.toLowerCase() === "premium" ||
        pro.plan.toLowerCase() === "annual" ||
        pro.plan.toLowerCase() === "monthly")
        ? "premium"
      ...
      const existing = await dbInstance
        ...
        .where(
          and(
            ...
            eq(favorites.providerId, input.providerId),
          ),
        )
        .limit(1);
