...
import { whatsappClicks, serviceViews, searchQueries, appEvents, providers, services } from "../../drizzle/schema";
...
export const analyticsRouter = router({
  ...
  trackServiceView: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.serviceId) {
        // Query providers
        const provider = await dbInstance.select().from(providers).where(eq(providers.id, input.serviceId)).limit(1);
        if (provider.length > 0) {
          city = provider[0].city || null;
          valor = provider[0].name || null;
        } else {
          // Query services (admin services)
          const service = await dbInstance.select().from(services).where(eq(services.id, input.serviceId)).limit(1);
          if (service.length > 0) {
  ...
  trackSearch: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!city && input.userId) {
        const provider = await dbInstance.select().from(providers).where(eq(providers.userId, input.userId)).limit(1);
        if (provider.length > 0) {
          city = provider[0].city || null;
        }
      }
