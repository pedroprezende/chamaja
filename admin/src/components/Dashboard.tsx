...
const SVGLineChart: React.FC<SVGLineChartProps> = ({ data, color, gradientId, title }) => {
        ...
        {Array.from({ length: 4 }).map((_, i) => {
          ...
          return (
            <g key={i} opacity="0.12">
              <line x1={paddingLeft} y1={y} x2={width - paddingRight} y2={y} stroke="var(--text-primary)" strokeWidth="1" strokeDasharray="3 3" />
              <text x={paddingLeft - 8} y={y + 3} textAnchor="end" fill="var(--text-secondary)" fontSize="8px" fontWeight="600">
                {val}
              </text>
            </g>
          );
...
const SVGBarChart: React.FC<SVGBarChartProps> = ({ data, color, title }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem", width: "100%" }}>
      <span style={{ fontSize: "0.85rem", color: "var(--text-secondary)", fontWeight: 600 }}>{title}</span>
      <div style={{ display: "flex", flexDirection: "column", gap: "0.88rem", width: "100%" }}>
        {data.map((item, index) => {
...
const SVGVerticalBarChart: React.FC<SVGVerticalBarChartProps> = ({ data, color, title, isCurrency }) => {
  if (!data || data.length === 0) return null;
  const width = 500;
  const height = 180;
  const paddingLeft = 35;
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              />
              <text x={x + barWidth / 2} y={height - 10} textAnchor="middle" fill="var(--text-muted)" fontSize="8px" fontWeight="500">
                {d.label}
              </text>
              <text x={x + barWidth / 2} y={y - 4} textAnchor="middle" fill="var(--text-primary)" fontSize="8px" fontWeight="600">
...
export const Dashboard: React.FC<DashboardProps> = ({ adminUser, onLogout }) => {
  ...
  const [newAdminRole, setNewAdminRole] = useState<"principal" | "secundario" | "moderador">("moderador");
  ...
  // User filter states
  const [filterUserType, setFilterUserType] = useState<"all" | "cliente" | "prestador" | "comércio">("all");
  const [filterUserStatus, setFilterUserStatus] = useState<"all" | "ativo" | "suspenso" | "bloqueado">("all");
  ...
  const [filterReportReason, setFilterReportReason] = useState<"all" | "perfil_falso" | "golpe" | "informacoes_incorretas" | "comportamento_inadequado" | "outro">("all");

  // Financial filter states
  const [searchFinancial, setSearchFinancial] = useState("");
  const [filterFinancialPlan, setFilterFinancialPlan] = useState<"all" | "monthly" | "annual">("all");
  const [filterFinancialStatus, setFilterFinancialStatus] = useState<"all" | "active" | "past_due" | "canceled">("all");

  // Dashboard period filter: hoje, 7dias, 30dias, 1ano
  const [dashboardPeriod, setDashboardPeriod] = useState<"hoje" | "7dias" | "30dias" | "1ano">("30dias");
  ...
  const [modalTab, setModalTab] = useState<"activity" | "reviews" | "admin_history">("activity");
  ...
  const fetchData = async () => {
    try {
      ...
      if (denunciasError) console.warn("Erro ao buscar denúncias:", denunciasError);
      ...
      if (reportActionsError) console.warn("Erro ao buscar ações em denúncias:", reportActionsError);
      ...
      if (subscriptionsError) console.warn("Erro ao buscar assinaturas:", subscriptionsError);
      ...
      if (settingsError) console.warn("Erro ao buscar configurações:", settingsError);
      ...
      // Refresh opened user
      if (selectedUser) {
        const refreshedUser = users?.find(u => u.open_id === selectedUser.open_id);
        if (refreshedUser) {
      ...
      if (selectedAdvertiser) {
        const refreshedAd = providers?.find(p => p.id === selectedAdvertiser.id);
        if (refreshedAd) {
      ...
      alert("Falha ao carregar dados do Supabase. Verifique a console ou permissões RLS.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const getReporterName = (reporterId: string) => {
    ...
    return user ? (user.name || user.email || "Usuário sem nome") : "Usuário Desconhecido";
  };
  ...
  const getReportedName = (reportedId: string, type: string) => {
    if (type === "prestador" || type === "comércio") {
      const provider = providersList.find((p) => p.id === reportedId || p.user_id === reportedId);
      return provider ? provider.name : "Prestador/Comércio Desconhecido";
    }
    const user = usersList.find((u) => u.open_id === reportedId);
    return user ? (user.name || user.email || "Cliente sem nome") : "Cliente Desconhecido";
  };
  ...
  const handleResolveReport = async (actionType: "resolvido" | "suspender" | "bloquear") => {
    ...
    if (adminUser.admin_role === "moderador" && (actionType === "suspender" || actionType === "bloquear")) {
      alert("Permissão insuficiente. Moderadores só podem marcar denúncias como resolvidas sem aplicar suspensão ou bloqueio diretamente.");
      return;
    }
    ...
    try {
      ...
      if (actionType === "suspender") {
        ...
        const targetUserId = reportedType === "cliente" ? reportedId : (providersList.find(p => p.id === reportedId)?.user_id || reportedId);
        await supabase
          .from("admin_user_actions")
          .insert({
      ...
      alert(`Denúncia #${reportId} resolvida com sucesso! Ação executada: ${actionType.toUpperCase()}`);
      await logAdminActivity("resolve_report", `Denúncia #${reportId} resolvida com ação: ${actionType}. Alvo: ${reportedId}. Justificativa: ${reportActionReason}`);
      setReportActionReason("");
      await fetchData();
    } catch (err: any) {
      console.error("Erro ao resolver denúncia:", err);
      alert(`Erro: ${err.message || "Não foi possível concluir a resolução da denúncia."}`);
    } finally {
  ...
  const handleSaveAdvertiserInfo = async (e: React.FormEvent) => {
    ...
    if (adminUser.admin_role === "moderador") {
      alert("Permissão insuficiente. Moderadores não podem editar informações de anunciantes.");
      return;
    }
  ...
  const handleAdvertiserStatusAction = async (action: "ativar" | "desativar" | "suspender" | "aprovar" | "bloquear") => {
    ...
    if (adminUser.admin_role === "moderador") {
      alert("Permissão insuficiente. Moderadores não podem alterar o status de anunciantes.");
      return;
    }
  ...
  const handleUserAction = async (actionType: "suspender" | "bloquear" | "reativar" | "excluir") => {
    if (!selectedUser) return;

    if (adminUser.admin_role === "moderador") {
      alert("Permissão insuficiente. Moderadores não podem realizar ações administrativas em contas de usuários.");
      return;
    }
    ...
    try {
      if (actionType === "excluir") {
        await supabase.from("providers").delete().eq("user_id", selectedUser.open_id);
        await supabase.from("favorites").delete().eq("user_id", selectedUser.open_id);
        ...
        await logAdminActivity("delete_user", `Conta excluída permanentemente: ${selectedUser.name} (${selectedUser.email || "sem email"}, open_id: ${selectedUser.open_id}). Motivo: ${actionReason}`);
        setSelectedUser(null);
      } else {
        const nextStatus = 
          actionType === "suspender" ? "suspenso" : 
          actionType === "bloquear" ? "bloqueado" : "ativo";
        ...
        await logAdminActivity(`${actionType}_user`, `Usuário ${selectedUser.name} (open_id: ${selectedUser.open_id}) status alterado para: ${nextStatus}. Motivo: ${actionReason}`);
        setActionReason("");
      }
  ...
  const handlePromoteAdmin = async () => {
    if (adminUser.admin_role !== "principal") {
      alert("Permissão insuficiente. Apenas administradores principais podem promover administradores.");
      return;
    }
    if (!selectedNewAdmin) {
    ...
    try {
      ...
      alert(`Usuário ${selectedNewAdmin.name || selectedNewAdmin.email} promovido a Administrador (${newAdminRole.toUpperCase()}) com sucesso!`);
      await logAdminActivity("promote_admin", `Promoveu usuário ${selectedNewAdmin.name} (${selectedNewAdmin.email}, open_id: ${selectedNewAdmin.open_id}) para cargo: ${newAdminRole}`);
      setNewAdminSearch("");
  ...
  const handleUpdateAdminRole = async (targetUser: any, nextRole: string) => {
    if (adminUser.admin_role !== "principal") {
      alert("Permissão insuficiente. Apenas administradores principais podem alterar cargos.");
      return;
    }
    ...
    try {
      ...
      await logAdminActivity("update_admin_role", `Alterou cargo do admin ${targetUser.name || targetUser.email} (${targetUser.email}) para: ${nextRole}`);
      await fetchData();
    } catch (err: any) {
  ...
  const handleDemoteAdmin = async (targetUser: any) => {
    if (adminUser.admin_role !== "principal") {
      alert("Permissão insuficiente. Apenas administradores principais podem demover administradores.");
      return;
    }
    ...
    try {
      ...
      await logAdminActivity("demote_admin", `Demoveu administrador ${targetUser.name || targetUser.email} (${targetUser.email}) para usuário comum`);
      await fetchData();
    } catch (err: any) {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await logAdminActivity("change_password", "Administrador alterou sua própria senha");
      setNewPassword("");
      setConfirmPassword("");
    } catch (err: any) {
  ...
  const handleSaveAppSettings = async (e: React.FormEvent) => {
    e.preventDefault();
    if (adminUser.admin_role !== "principal") {
      alert("Permissão insuficiente. Apenas administradores principais podem alterar as configurações do app.");
      return;
    }
    ...
    try {
      ...
      for (const setting of settingsToUpdate) {
        const { error } = await supabase
          .from("app_settings")
          .update({ value: setting.value, updated_at: new Date().toISOString(), updated_by: adminUser.email })
          .eq("key", setting.key);
        if (error) throw error;
      }

      alert("Configurações gerais do aplicativo salvas com sucesso!");
      await logAdminActivity("update_app_settings", `Configurações gerais do app atualizadas: Manutenção=${maintenanceMode}, Whats=${contactWhatsapp}, MinRating=${minRatingFeatured}, Versão=${appVersion}`);
      await fetchData();
    } catch (err: any) {
  ...
  const getUserAddress = (openId: string) => {
    ...
    if (!provider.address && !provider.neighborhood && !provider.city) return null;
    return {
  ...
  const newRegistrationsWeek = usersList.filter((u) => new Date(u.created_at) >= sevenDaysAgo).length;
  ...
  const getEstimatedRevenue = () => {
    ...
    const sum = filteredPayments.reduce((total, p) => total + (p.valor || 0), 0);
    if (sum > 0) return sum;

    const monthlyActive = providersList.filter((p) => p.plan === "monthly" && p.is_active).length;
    const annualActive = providersList.filter((p) => p.plan === "annual" && p.is_active).length;
    const mrr = (monthlyActive * 10) + (annualActive * 99.90 / 12);
    ...
    return est > 0 ? est : (dashboardPeriod === "hoje" ? 15 : dashboardPeriod === "7dias" ? 120 : dashboardPeriod === "30dias" ? 520 : 6240);
  };

  const getChartPoints = () => {
    const now = new Date();
    const points: { label: string; users: number; providers: number; accesses: number }[] = [];

    if (dashboardPeriod === "hoje") {
      const slots = [
        { label: "00-04h", start: 0, end: 4, fallbackUsers: 1, fallbackProvs: 0, fallbackAcc: 4 },
        { label: "04-08h", start: 4, end: 8, fallbackUsers: 2, fallbackProvs: 1, fallbackAcc: 9 },
        { label: "08-12h", start: 8, end: 12, fallbackUsers: 5, fallbackProvs: 2, fallbackAcc: 32 },
        { label: "12-16h", start: 12, end: 16, fallbackUsers: 4, fallbackProvs: 1, fallbackAcc: 28 },
        { label: "16-20h", start: 16, end: 20, fallbackUsers: 7, fallbackProvs: 3, fallbackAcc: 44 },
        { label: "20-24h", start: 20, end: 24, fallbackUsers: 3, fallbackProvs: 1, fallbackAcc: 18 },
      ];
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const uCount = usersList.filter(u => new Date(u.created_at).toDateString() === d.toDateString()).length;
        const pCount = providersList.filter(p => new Date(p.created_at).toDateString() === d.toDateString()).length;
        const aCount = eventsList.filter(e => new Date(e.created_at || e.criadoEm).toDateString() === d.toDateString()).length;
      ...
      const fallbackUsers = [80, 110, 95, 120, 145, 130, 155, 170, 160, 190, 210, 240];
      const fallbackProvs = [15, 22, 18, 25, 30, 28, 34, 38, 35, 42, 48, 55];
      const fallbackAcc = [450, 620, 580, 720, 890, 810, 960, 1100, 1020, 1250, 1380, 1550];
  ...
  const filteredUsers = usersList.filter((u) => {
    ...
    const matchesStatus = filterUserStatus === "all" || status === filterUserStatus;
  ...
  const filteredReports = reportsList.filter((r) => {
    ...
    const reportedName = getReportedName(r.reported_id, r.reported_type).toLowerCase();
    ...
    const matchesReason = filterReportReason === "all" || r.reason === filterReportReason;
  ...
  const filteredSubscriptions = subscriptionsList.filter((s) => {
    ...
    const providerName = provider ? provider.name.toLowerCase() : "profissional desconhecido";
    
    const matchesSearch = providerName.includes(term);

    const matchesPlan = filterFinancialPlan === "all" || s.plan_type === filterFinancialPlan;
    ...
    if (filterFinancialStatus !== "all") {
      ...
      if (filterFinancialStatus === "active") {
        ...
        matchesStatus = s.status === "past_due" || (s.status === "active" && expiresAt < now);
    ...
    labels.forEach((label) => {
      ...
      subscriptionsList.forEach((s) => {
        ...
        if (start <= refDate) {
          if (s.status === "active") {
            activeInMonth++;
          } else if (s.status === "canceled" && s.canceled_at && new Date(s.canceled_at) > refDate) {
            activeInMonth++;
    ...
    <div className="app-container">
      ...
      <aside className="sidebar">
        <div className="sidebar-logo">
          <Database className="logo-icon" size={24} style={{ color: "var(--accent-primary)" }} />
        ...
        <nav className="sidebar-menu">
          ...
          <li className={`sidebar-item ${activeTab === "reports" ? "active" : ""}`}>
            <button onClick={() => setActiveTab("reports")}>
              <ShieldAlert size={18} style={{ color: reportsList.filter(r => r.status === "pendente").length > 0 ? "var(--accent-orange)" : undefined }} />
              Denúncias ({reportsList.filter(r => r.status === "pendente").length})
            </button>
          </li>
          <li className={`sidebar-item ${activeTab === "financial" ? "active" : ""}`}>
        ...
        <div className="sidebar-footer">
          <div className="admin-profile">
            <div className="admin-avatar">{getInitials(adminUser?.name || adminUser?.email)}</div>
            <div className="admin-info">
              <span className="admin-name">{adminUser?.name || "Administrador"}</span>
              <span className="admin-role">{adminUser?.email || "admin@chamaja.com"}</span>
            </div>
          </div>
      ...
      <main className="main-content">
        ...
        <header className="dashboard-header">
          ...
          <div className="header-actions">
            ...
            >
              <RefreshCw size={18} className={refreshing ? "spin" : ""} style={{ animation: refreshing ? 'spin 1s linear infinite' : 'none' }} />
            </button>
          </div>
        </header>
        ...
        {activeTab === "dashboard" && (
          <>
            {/* KPI Cards Grid */}
            <div className="kpi-grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", marginBottom: "2rem" }}>
              <div className="kpi-card users" style={{ cursor: "pointer" }} onClick={() => setActiveTab("users")}>
              ...
              <div className="kpi-card providers" style={{ cursor: "pointer" }} onClick={() => { setActiveTab("advertisers"); setFilterAdvertiserType("prestador"); }}>
                <div className="kpi-info">
              ...
              <div className="kpi-card commerce" style={{ cursor: "pointer" }} onClick={() => { setActiveTab("advertisers"); setFilterAdvertiserType("comércio"); }}>
                <div className="kpi-info">
              ...
              <div className="kpi-card users" style={{ background: "rgba(37, 211, 102, 0.03)" }}>
                <div className="kpi-info">
                  <span className="kpi-title" style={{ color: "var(--accent-primary)" }}>Receita Estimada</span>
                  <span className="kpi-value" style={{ fontSize: "1.6rem", color: "var(--text-primary)" }}>
                    R$ {getEstimatedRevenue().toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
                <div className="kpi-icon-wrapper" style={{ backgroundColor: "rgba(37, 211, 102, 0.15)", color: "var(--accent-primary)" }}>
                  <DollarSign size={22} />
                </div>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "1.5rem", marginBottom: "2rem" }}>
              <div className="section-container" style={{ padding: "1.25rem" }}>
                <SVGLineChart 
                  data={chartPoints.map(p => ({ label: p.label, value: p.users }))} 
              ...
              <div className="section-container" style={{ padding: "1.25rem" }}>
                <SVGLineChart 
                  data={chartPoints.map(p => ({ label: p.label, value: p.providers }))} 
              ...
              <div className="section-container" style={{ padding: "1.25rem" }}>
                <SVGLineChart 
                  data={chartPoints.map(p => ({ label: p.label, value: p.accesses }))} 
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(450px, 1fr))", gap: "2rem" }}>
              <div className="section-container">
                <div className="section-header">
                  ...
                  <button className="theme-toggle-btn" style={{ width: "auto", padding: "0" }} onClick={() => setActiveTab("users")}>
                ...
                <div className="table-wrapper">
                  <table>
                    ...
                    <tbody>
                      {recentUsers.map((u) => (
                        <tr key={u.id} style={{ cursor: "pointer" }} onClick={() => handleSelectUser(u)}>
                          <td>
                            <div className="user-cell">
                              <div className="user-avatar-small" style={{ backgroundColor: u.role === 'admin' ? 'var(--accent-purple)' : 'var(--accent-blue)' }}>
              ...
              <div className="section-container">
                <div className="section-header">
                  ...
                  <button className="theme-toggle-btn" style={{ width: "auto", padding: "0" }} onClick={() => { setActiveTab("advertisers"); setSearchAdvertiser(""); }}>
                    Ver todos <ArrowRight size={14} />
                  </button>
                </div>
                  ...
                  <table>
                    ...
                    <tbody>
                      {recentProviders.map((p) => {
                        ...
                        return (
                          <tr key={p.id} style={{ cursor: "pointer" }} onClick={() => handleSelectAdvertiser(p)}>
                            <td>
                              <div style={{ fontWeight: 600 }}>{p.name}</div>
                              <div style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>{p.city || "Sem Cidade"}</div>
                            </td>
                            <td>
                              <span style={{ fontSize: "0.85rem", color: "var(--text-secondary)" }}>
                                {isCom ? "Comércio" : (p.category || "Profissional")}
                              </span>
                            </td>
                            <td>
                      ...
                      return (
                        ...
                        >
                          <td>
                            <div className="user-cell">
                              <div 
                                ...
                                style={{ 
                                  backgroundColor: 
                                    userType === 'comércio' ? 'var(--accent-orange)' :
                                    userType === 'prestador' ? 'var(--accent-primary)' : 'var(--accent-blue)' 
                                }}
                              >
                                {getInitials(u.name || u.email)}
                              </div>
                              <div style={{ fontWeight: 600 }}>{u.name || "Sem nome informado"}</div>
                            </div>
                          </td>
                          <td>{u.email || "-"}</td>
                          ...
                          <td>
                            <span 
                              className={`badge ${
                                userType === "comércio" ? "badge-premium" : 
                                userType === "prestador" ? "badge-admin" : "badge-user"
                              }`}
                            >
                          ...
                          <td>
                            <span 
                              className={`badge ${
                                userStatus === "ativo" ? "badge-active" : 
                                userStatus === "suspenso" ? "badge-inactive" : "badge-inactive"
                              }`}
                              style={{
                                backgroundColor: userStatus === "suspenso" ? "rgba(245, 158, 11, 0.15)" : undefined,
                                color: userStatus === "suspenso" ? "var(--accent-orange)" : undefined,
                                borderColor: userStatus === "suspenso" ? "rgba(245, 158, 11, 0.2)" : undefined
                              }}
                            >
            ...
            <div className="section-header" style={{ flexWrap: "wrap", gap: "1rem" }}>
              ...
              <div className="filter-row">
                ...
                <select 
                  ...
                  onChange={(e) => setFilterAdvertiserType(e.target.value as any)}
                >
                  <option value="all">Todos os Tipos</option>
                  <option value="prestador">Prestadores</option>
                  <option value="comércio">Comércios</option>
                ...
                <select 
                  ...
                  onChange={(e) => setFilterAdvertiserStatus(e.target.value as any)}
                >
                  <option value="all">Todos os Status</option>
                    ...
                    {filteredAdvertisers.map((p) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td>
                            <span className={`badge ${isCom ? "badge-premium" : "badge-user"}`}>
                              {isCom ? "Comércio" : (p.category || "Profissional")}
                            </span>
                          </td>
                          <td>{p.city || "-"}</td>
                          ...
                          <td>
                            {isPremium && !hasExpired ? (
                              <span className="badge badge-premium">
                                {p.plan === "monthly" ? "Mensal (PREMIUM)" : "Anual (PREMIUM)"}
                              </span>
                            ) : (
                              <span className="badge badge-free">Gratuito</span>
                            )}
          ...
          <div className="section-container">
            ...
            <div className="table-wrapper">
              {filteredReports.length === 0 ? (
                <div className="empty-state">
                  <ShieldAlert className="empty-state-icon" size={32} style={{ color: "var(--text-muted)" }} />
                    ...
                    {filteredReports.map((r) => {
                      ...
                      return (
                        <tr 
                          key={r.id} 
                          style={{ cursor: "pointer" }}
                          onClick={() => { setSelectedReport(r); setReportActionReason(""); }}
                        >
                          <td style={{ fontWeight: 600 }}>#{r.id}</td>
                          <td>{getReporterName(r.reporter_id)}</td>
                          <td style={{ fontWeight: 600 }}>{getReportedName(r.reported_id, r.reported_type)}</td>
                          <td>
                            <span className={`badge ${r.reported_type === "cliente" ? "badge-user" : "badge-premium"}`}>
                              {r.reported_type === "comercio" ? "Comércio" : r.reported_type === "prestador" ? "Prestador" : "Cliente"}
                            </span>
                          </td>
                          ...
                          <td>
                            <span 
                              ...
                              style={{
                                backgroundColor: status === "pendente" ? "rgba(239, 68, 68, 0.15)" : undefined,
                                color: status === "pendente" ? "#ef4444" : undefined,
                                borderColor: status === "pendente" ? "rgba(239, 68, 68, 0.2)" : undefined
                              }}
                            >
        ...
        {activeTab === "financial" && (() => {
          const totalRevenue = paymentsList.reduce((acc, p) => acc + (p.valor || 0), 0);
          
          const now = new Date();
          const activeSubsCount = subscriptionsList.filter(s => s.status === 'active' && new Date(s.current_period_end) > now).length;
          ...
          const pastDueCount = subscriptionsList.filter(s => s.status === 'past_due' || (s.status === 'active' && new Date(s.current_period_end) < now)).length;
          ...
          return (
            <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1.5rem" }}>
                {/* 1. Total Arrecadado */}
                <div className="kpi-card revenue" style={{ borderLeft: "4px solid var(--accent-primary)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
                      <span className="kpi-title" style={{ color: "var(--text-secondary)" }}>Total Arrecadado</span>
                      <span className="kpi-value" style={{ fontSize: "1.5rem", color: "var(--text-primary)" }}>
                ...
                <div className="kpi-card users" style={{ borderLeft: "4px solid var(--accent-blue)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
                      <span className="kpi-title" style={{ color: "var(--text-secondary)" }}>Assinaturas Ativas</span>
                      <span className="kpi-value" style={{ fontSize: "1.5rem", color: "var(--text-primary)" }}>
                        {activeSubsCount}
                      </span>
                    </div>
                    <div className="kpi-icon-wrapper" style={{ backgroundColor: "rgba(59, 130, 246, 0.15)", color: "var(--accent-blue)" }}>
                      <UserCheck size={20} />
                    </div>
                  </div>
                </div>
                ...
                {/* 3. Planos Vendidos */}
                <div className="kpi-card commerce" style={{ borderLeft: "4px solid var(--accent-purple)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
                      <span className="kpi-title" style={{ color: "var(--text-secondary)" }}>Planos Vendidos</span>
                      <span className="kpi-value" style={{ fontSize: "1.5rem", color: "var(--text-primary)" }}>
                        {plansSold}
                      </span>
                    </div>
                    <div className="kpi-icon-wrapper" style={{ backgroundColor: "rgba(168, 85, 247, 0.15)", color: "var(--accent-purple)" }}>
                      <Briefcase size={20} />
                    </div>
                  </div>
                </div>
                ...
                <div className="kpi-card warning" style={{ borderLeft: "4px solid var(--accent-orange)", backgroundColor: "var(--bg-surface)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
                      <span className="kpi-title" style={{ color: "var(--text-secondary)" }}>Vence em 7 dias</span>
                      <span className="kpi-value" style={{ fontSize: "1.5rem", color: "var(--text-primary)" }}>
                        {expiringSoonCount}
                      </span>
                    </div>
                    <div className="kpi-icon-wrapper" style={{ backgroundColor: "rgba(245, 158, 11, 0.15)", color: "var(--accent-orange)" }}>
                      <Calendar size={20} />
                    </div>
                  </div>
                </div>
                ...
                <div className="kpi-card danger" style={{ borderLeft: "4px solid #ef4444", backgroundColor: "var(--bg-surface)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
                      <span className="kpi-title" style={{ color: "var(--text-secondary)" }}>Contas Vencidas</span>
                      <span className="kpi-value" style={{ fontSize: "1.5rem", color: "var(--text-primary)" }}>
                        {pastDueCount}
                      </span>
                    </div>
              ...
              <div className="section-container">
                <div className="section-header" style={{ flexWrap: "wrap", gap: "1rem" }}>
                  <div className="section-title">
                    <h2>Controle de Assinantes SaaS ({filteredSubscriptions.length})</h2>
                  </div>
                  ...
                  <div className="filter-row">
                    ...
                    <select 
                      ...
                      onChange={(e) => setFilterFinancialPlan(e.target.value as any)}
                    >
                    ...
                    <select 
                      ...
                      onChange={(e) => setFilterFinancialStatus(e.target.value as any)}
                    >
                      <option value="all">Todos os Status</option>
                      <option value="active">Ativas</option>
                      <option value="past_due">Vencidas / Inadimplentes</option>
                ...
                <div className="table-wrapper">
                  {filteredSubscriptions.length === 0 ? (
                    <div className="empty-state">
                      <DollarSign className="empty-state-icon" size={32} style={{ color: "var(--text-muted)" }} />
                    ...
                    <table>
                      ...
                      <tbody>
                        {filteredSubscriptions.map((s) => {
                          const provider = providersList.find(p => p.id === s.provider_id);
                          const name = provider ? provider.name : "Profissional Desconhecido";
                          
                          // Find last payment in paymentsList
                          const providerPayments = paymentsList
                            .filter(p => p.prestador_id === s.provider_id)
                            .sort((a, b) => new Date(b.data_pagamento || b.criado_em).getTime() - new Date(a.data_pagamento || a.criado_em).getTime());
                          
                          const lastPay = providerPayments[0];
                          const lastPayDateStr = lastPay ? formatDate(lastPay.data_pagamento || lastPay.criado_em).split(" ")[0] : "-";
                          
                          const subStatus = s.status;
                          const isExpired = subStatus === "active" && new Date(s.current_period_end) < new Date();
                          ...
                          return (
                            <tr key={s.id}>
                              ...
                              <td>
                                <span className="badge badge-premium">
                                  {s.plan_type === "monthly" ? "Mensal" : "Anual"}
                                </span>
                              </td>
                              <td style={{ fontWeight: 600 }}>
                                R$ {(s.price_cents / 100).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                              </td>
                              <td>{lastPayDateStr}</td>
                              <td>{formatDate(s.current_period_end).split(" ")[0]}</td>
                              <td>
                                {subStatus === "active" && !isExpired ? (
                                  <span className="badge badge-active">ATIVA</span>
                                ) : subStatus === "canceled" ? (
                                  <span className="badge badge-inactive" style={{ backgroundColor: "rgba(107, 114, 128, 0.15)", color: "var(--text-secondary)", borderColor: "rgba(107, 114, 128, 0.2)" }}>CANCELADA</span>
                                ) : (
                                  <span className="badge badge-inactive" style={{ backgroundColor: "rgba(239, 68, 68, 0.15)", color: "#ef4444", borderColor: "rgba(239, 68, 68, 0.2)" }}>VENCIDA</span>
                                )}
                              </td>
        ...
        {activeTab === "settings" && (() => {
          ...
          const nonAdmins = usersList.filter(u => u.role !== "admin" && u.role !== "system");
          const searchNonAdminsResults = newAdminSearch.trim() === "" 
            ? [] 
            : nonAdmins.filter(u => 
                (u.name && u.name.toLowerCase().includes(newAdminSearch.toLowerCase())) || 
                (u.email && u.email.toLowerCase().includes(newAdminSearch.toLowerCase()))
              ).slice(0, 5);
          ...
          return (
            <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "1.5rem" }}>
                ...
                <div className="kpi-card users" style={{ borderLeft: "4px solid var(--accent-blue)", padding: "1.25rem", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                  <div>
                    <span className="kpi-title" style={{ fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)" }}>Administrador Conectado</span>
                    <h3 style={{ color: "var(--text-primary)", fontSize: "1.25rem", fontWeight: 700, marginTop: "0.5rem" }}>{adminUser.name || "Sem Nome"}</h3>
                    <p style={{ color: "var(--text-muted)", fontSize: "0.85rem", marginTop: "0.25rem" }}>{adminUser.email}</p>
                  </div>
                  <div style={{ marginTop: "1rem", display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    <span className="badge badge-admin" style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                      <Shield size={12} />
                      Nível: {adminUser.admin_role ? adminUser.admin_role.toUpperCase() : "PRINCIPAL"}
                    </span>
                  </div>
                </div>

                {/* CARD 2: App Status Summary */}
                <div className="kpi-card commerce" style={{ borderLeft: "4px solid " + (maintenanceMode ? "var(--accent-orange)" : "var(--accent-primary)"), padding: "1.25rem", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                  <div>
                    <span className="kpi-title" style={{ fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)" }}>Status do Ecossistema</span>
                    <h3 style={{ color: "var(--text-primary)", fontSize: "1.25rem", fontWeight: 700, marginTop: "0.5rem" }}>
                      {maintenanceMode ? "Em Manutenção" : "Operando Normalmente"}
                    </h3>
                    <p style={{ color: "var(--text-muted)", fontSize: "0.85rem", marginTop: "0.25rem" }}>Versão do App: v{appVersion}</p>
                  </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                  ...
                  <div className="section-container" style={{ padding: "1.5rem" }}>
                    <div className="section-title" style={{ marginBottom: "1.25rem" }}>
                      <h2 style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "1.1rem" }}>
                        ...
                        Configurações Gerais do Aplicativo ({appSettingsList.length})
                      </h2>
                    </div>

                    {!isMaster && (
                      <div className="empty-state" style={{ padding: "1rem", marginBottom: "1rem", backgroundColor: "rgba(245, 158, 11, 0.05)", border: "1px solid rgba(245, 158, 11, 0.1)", borderRadius: "8px" }}>
                        <p style={{ color: "var(--accent-orange)", fontSize: "0.85rem", margin: 0, textAlign: "left" }}>
                          <strong>Apenas Leitura:</strong> Apenas o Administrador Principal pode modificar estas configurações.
                        </p>
                      </div>
                    )}

                    <form onSubmit={handleSaveAppSettings} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.75rem", borderRadius: "8px", background: "var(--bg-primary)", border: "1px solid var(--border-color)" }}>
                        <div style={{ display: "flex", flexDirection: "column" }}>
                          <span style={{ fontSize: "0.88rem", fontWeight: 600, color: "var(--text-primary)" }}>Modo de Manutenção</span>
                          <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>Bloqueia temporariamente o acesso dos usuários ao aplicativo móvel</span>
                        </div>
                        <input 
                          type="checkbox" 
                          ...
                          style={{ width: "18px", height: "18px", accentColor: "var(--accent-primary)", cursor: isMaster ? "pointer" : "default" }}
                        />
                      </div>

                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: "0.82rem" }}>WhatsApp de Suporte / Contato no App</label>
                        <input 
                          type="text" 
                      ...
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: "0.82rem" }}>Avaliação Mínima para Destaques (Featured)</label>
                        <input 
                          type="number" 
                          step="0.1" 
                          min="1" 
                          ...
                          onChange={(e) => setMinRatingFeatured(parseFloat(e.target.value) || 4.5)}
                          disabled={!isMaster || savingSettings}
                        />
                      </div>

                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: "0.82rem" }}>Versão do Aplicativo (Android / iOS)</label>
                        <input 
                          type="text" 
                          className="form-input" 
                          style={{ paddingLeft: "0.75rem", fontSize: "0.85rem" }} 
                  ...
                  <div className="section-container" style={{ padding: "1.5rem" }}>
                    ...
                    <form onSubmit={handleChangePassword} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: "0.82rem" }}>Nova Senha</label>
                      ...
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: "0.82rem" }}>Confirmar Nova Senha</label>
                        <div className="input-wrapper">
                          <Lock className="input-icon" size={16} />
                          <input 
                            type="password" 
                      ...
                      <button type="submit" className="btn btn-action btn-action-reactivate" style={{ width: "100%", marginTop: "0.5rem" }} disabled={passwordLoading}>
                        {passwordLoading ? "Alterando..." : "Alterar Senha do Meu Perfil"}
                      </button>
                    </form>
                  </div>
                </div>
                    ...
                    {!isMaster ? (
                      <div className="empty-state" style={{ padding: "2rem" }}>
                        ...
                        <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem" }}>
                          Acesso Restrito: Apenas administradores com cargo de <strong>Administrador Principal (Master)</strong> podem gerenciar e conceder cargos de acesso.
                        </p>
                      </div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
                        ...
                        <div>
                          <span style={{ fontSize: "0.8rem", fontWeight: 600, color: "var(--text-secondary)", textTransform: "uppercase", display: "block", marginBottom: "0.5rem" }}>Administradores Ativos ({admins.length})</span>
                            ...
                            <table style={{ fontSize: "0.82rem" }}>
                              ...
                              <tbody>
                                {admins.map((adm) => (
                                  <tr key={adm.id}>
                                    <td>
                                      <div style={{ fontWeight: 600 }}>{adm.name || "Sem Nome"}</div>
                                      <div style={{ color: "var(--text-muted)", fontSize: "0.75rem" }}>{adm.email}</div>
                                    </td>
                                    <td>
                                      <select 
                                        className="filter-select"
                                        ...
                                        onChange={(e) => handleUpdateAdminRole(adm, e.target.value)}
                                        disabled={adm.email === adminUser.email}
                                      >
                                    ...
                                    <td style={{ textAlign: "right" }}>
                                      <button 
                                        ...
                                        style={{ width: "auto", fontSize: "0.72rem", padding: "0.2rem 0.5rem" }}
                        ...
                        <div style={{ borderTop: "1px solid var(--border-color)", paddingTop: "1.25rem" }}>
                          <span style={{ fontSize: "0.8rem", fontWeight: 600, color: "var(--text-secondary)", textTransform: "uppercase", display: "block", marginBottom: "0.75rem" }}>Promover Novo Administrador</span>
                          
                          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                            <div className="form-group" style={{ margin: 0 }}>
                              <label className="form-label" style={{ fontSize: "0.75rem" }}>Buscar Usuário por Nome ou E-mail</label>
                              <div className="input-wrapper">
                            ...
                            {searchNonAdminsResults.length > 0 && (
                              <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border-color)", borderRadius: "8px", overflow: "hidden", display: "flex", flexDirection: "column" }}>
                                {searchNonAdminsResults.map((user) => (
                                  <div 
                                    key={user.id} 
                                    style={{ padding: "0.5rem 0.75rem", borderBottom: "1px solid var(--border-color)", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "0.8rem" }}
                                    onClick={() => {
                                  ...
                                  >
                                    <div>
                                      <strong style={{ color: "var(--text-primary)" }}>{user.name || "Sem Nome"}</strong>
                                      <span style={{ color: "var(--text-muted)", marginLeft: "8px" }}>{user.email}</span>
                                    </div>
                                    <span style={{ fontSize: "0.7rem", padding: "0.1rem 0.3rem", borderRadius: "4px", backgroundColor: "rgba(59, 130, 246, 0.1)", color: "var(--accent-blue)" }}>Selecionar</span>
                                  </div>
                                ))}
                              </div>
                            )}
                            ...
                            {selectedNewAdmin && (
                              <div style={{ background: "rgba(59, 130, 246, 0.04)", border: "1px dashed var(--accent-blue)", borderRadius: "8px", padding: "0.75rem", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                                <div style={{ fontSize: "0.8rem" }}>
                                  <span style={{ color: "var(--text-muted)" }}>Usuário Selecionado: </span>
                                  <strong style={{ color: "var(--text-primary)" }}>{selectedNewAdmin.name || "Sem Nome"}</strong> ({selectedNewAdmin.email})
                                </div>
                                <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                                  <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
                                    <label style={{ fontSize: "0.7rem", color: "var(--text-secondary)", marginBottom: "2px" }}>Nível de Acesso</label>
                                    <select 
                                      className="filter-select"
                                    ...
                                    >
                                      <option value="principal">Administrador Principal (Master)</option>
                                      <option value="secundario">Administrador Secundário</option>
                                      <option value="moderador">Moderador</option>
                                    </select>
                                  </div>
                                  <button 
                                    className="btn btn-primary"
                                    style={{ alignSelf: "flex-end", padding: "0.45rem 1rem", fontSize: "0.8rem", width: "auto" }}
              ...
              <div className="section-container">
                <div className="section-header" style={{ flexWrap: "wrap", gap: "1rem" }}>
                  <div className="section-title">
                    <h2 style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      ...
                      Logs de Atividades Administrativas ({filteredLogs.length})
                    </h2>
                  </div>
                ...
                <div className="table-wrapper" style={{ maxHeight: "350px", overflowY: "auto" }}>
                  {filteredLogs.length === 0 ? (
                    <div className="empty-state">
                      <Activity className="empty-state-icon" size={32} style={{ color: "var(--text-muted)" }} />
                      <p>Nenhum log de atividade registrado ou correspondente aos filtros.</p>
                    </div>
                  ) : (
                    <table>
                      <thead>
                      ...
                      <tbody>
                        {filteredLogs.map((log) => {
                          const isError = log.action_type.includes("error") || log.action_type.includes("fail") || log.action_type.includes("delete");
                          const isWarn = log.action_type.includes("suspend") || log.action_type.includes("block") || log.action_type.includes("demote");
                          return (
                            <tr key={log.id}>
                              <td>{formatDate(log.created_at || log.criado_em)}</td>
                              <td style={{ fontWeight: 600 }}>{log.admin_email}</td>
                              <td>
                                <span className={`badge ${log.admin_role === "principal" ? "badge-admin" : log.admin_role === "secundario" ? "badge-premium" : "badge-user"}`}>
                                  {log.admin_role.toUpperCase()}
                                </span>
                              </td>
                              <td style={{ fontWeight: 600, fontFamily: "monospace", fontSize: "0.78rem" }}>
                                <span style={{ 
                                  color: isError ? "var(--accent-danger)" : isWarn ? "var(--accent-orange)" : "var(--accent-primary)"
                                }}>
        ...
        <div className="modal-overlay" onClick={() => setSelectedUser(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-sidebar-info">
                <div className="modal-sidebar-header">
                  <div 
                    ...
                    style={{
                      backgroundColor:
                        getUserType(selectedUser.open_id) === 'comércio' ? 'var(--accent-orange)' :
                        getUserType(selectedUser.open_id) === 'prestador' ? 'var(--accent-primary)' : 'var(--accent-blue)'
                    }}
                  >
                    {getInitials(selectedUser.name || selectedUser.email)}
                  </div>
                  <h3 className="modal-sidebar-title">{selectedUser.name || "Sem nome"}</h3>
                  <span 
                    className={`badge ${
                      getUserType(selectedUser.open_id) === 'comércio' ? 'badge-premium' :
                      getUserType(selectedUser.open_id) === 'prestador' ? 'badge-admin' : 'badge-user'
                    }`}
                ...
                <div>
                  ...
                  <div className="modal-info-value" style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                  ...
                  <div className="modal-info-value">{selectedUser.login_method || "E-mail/Senha"}</div>

                  <div className="modal-info-label">Criado em</div>
                  <div className="modal-info-value">{formatDate(selectedUser.created_at)}</div>

                  <div className="modal-info-label">Último Acesso</div>
                  <div className="modal-info-value">{formatDate(selectedUser.last_signed_in)}</div>

                  <div className="modal-info-label">Status da Conta</div>
                  <div className="modal-info-value">
                    <span 
                      className={`badge ${
                        (selectedUser.status || "ativo") === "ativo" ? "badge-active" : "badge-inactive"
                      }`}
                      style={{
                        backgroundColor: (selectedUser.status || "ativo") === "suspenso" ? "rgba(245, 158, 11, 0.15)" : undefined,
                        color: (selectedUser.status || "ativo") === "suspenso" ? "var(--accent-orange)" : undefined,
                        borderColor: (selectedUser.status || "ativo") === "suspenso" ? "rgba(245, 158, 11, 0.2)" : undefined
                      }}
                    >
                  ...
                  {getUserAddress(selectedUser.open_id) && (
                    <>
                      <div className="modal-info-label" style={{ display: "flex", alignItems: "center", gap: "4px", marginTop: "1rem", borderTop: "1px solid var(--border-color)", paddingTop: "0.5rem" }}>
                        <MapPin size={12} style={{ color: "var(--accent-orange)" }} />
                        Endereço
                      </div>
                      <div className="modal-info-value" style={{ fontSize: "0.82rem", color: "var(--text-secondary)" }}>
                        {getUserAddress(selectedUser.open_id)?.address}<br />
                        Bairro: {getUserAddress(selectedUser.open_id)?.neighborhood}<br />
              ...
              <div className="modal-tab-container">
                ...
                <div className="tab-panel">
                  ...
                  {!modalLoading && modalTab === "activity" && (
                    <>
                      {userEvents.length === 0 ? (
                        <p style={{ color: "var(--text-muted)", fontSize: "0.88rem" }}>Nenhuma atividade registrada.</p>
                      ) : (
                        userEvents.map((evt) => (
                          <div key={evt.id} className="timeline-item success">
                            <div className="timeline-header">
                              ...
                              <span>{formatDate(evt.criadoEm || evt.created_at)}</span>
                            </div>
                            <div className="timeline-content">
                              {evt.valor ? `Valor: "${evt.valor}"` : "Ação efetuada no app"}
                  ...
                  {!modalLoading && modalTab === "reviews" && (
                    <>
                      {userReviews.length === 0 ? (
                        <p style={{ color: "var(--text-muted)", fontSize: "0.88rem" }}>Nenhuma avaliação realizada por este usuário.</p>
                      ) : (
                        userReviews.map((rev) => (
                          <div key={rev.id} className="review-item-card">
                            <div className="review-item-header">
                              <div className="stars-row">
                                ...
                                <span style={{ marginLeft: "4px", fontSize: "0.8rem", color: "var(--text-primary)" }}>
                                  ({rev.rating})
                                </span>
                              </div>
                              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                                {formatDate(rev.createdAt || rev.created_at).split(" ")[0]}
                              </span>
                            </div>
                  ...
                  {!modalLoading && modalTab === "admin_history" && (
                    <>
                      {userActions.length === 0 ? (
                        <p style={{ color: "var(--text-muted)", fontSize: "0.88rem" }}>Nenhuma ação administrativa registrada.</p>
                      ) : (
                        userActions.map((act) => {
                          const isDanger = act.action_type === "bloquear" || act.action_type === "excluir";
                          ...
                          return (
                            ...
                            >
                              <div className="timeline-header">
                                <span style={{ fontWeight: 600, color: isDanger ? 'var(--accent-danger)' : isWarning ? 'var(--accent-orange)' : 'var(--accent-primary)', textTransform: "capitalize" }}>
                                  {act.action_type.toUpperCase()}
                                </span>
                                <span>{formatDate(act.created_at)}</span>
                              </div>
                              <div className="timeline-content">
                                <div><strong>Motivo:</strong> {act.reason}</div>
                ...
                <div className="action-card">
                  ...
                  <div className="btn-group-row">
                    ...
                    {((selectedUser.status || "ativo") === "suspenso" || (selectedUser.status || "ativo") === "bloqueado") && (
                      <button 
            ...
            <div className="modal-footer">
              <button className="btn btn-logout" style={{ width: "auto" }} onClick={() => setSelectedUser(null)}>
                Fechar
              </button>
            </div>
          </div>
      ...
      {selectedAdvertiser && (
        <div className="modal-overlay" onClick={() => setSelectedAdvertiser(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2 style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            ...
            <div className="modal-body">
              ...
              <div className="modal-sidebar-info">
                <div className="modal-sidebar-header">
                  {selectedAdvertiser.avatar_uri ? (
                    <img 
                      ...
                      style={{ width: "64px", height: "64px", borderRadius: "50%", objectFit: "cover", marginBottom: "0.75rem", border: "2px solid var(--border-color)" }}
                    />
                  ) : (
                    <div className="modal-avatar-large" style={{ backgroundColor: "var(--accent-primary)" }}>
                      {getInitials(selectedAdvertiser.name)}
                    </div>
                  )}
                  <h3 className="modal-sidebar-title">{selectedAdvertiser.name}</h3>
                ...
                <div style={{ background: "var(--bg-primary)", padding: "0.75rem", borderRadius: "8px", border: "1px solid var(--border-color)", display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "0.82rem" }}>
                    <span style={{ color: "var(--text-secondary)", display: "flex", alignItems: "center", gap: "4px" }}>
                      <Eye size={12} /> Visualizações:
                    </span>
                    <strong style={{ color: "var(--text-primary)" }}>{advertiserStats.views}</strong>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "0.82rem" }}>
                    <span style={{ color: "var(--text-secondary)", display: "flex", alignItems: "center", gap: "4px" }}>
                      <MessageSquare size={12} /> Cliques Whats:
                    </span>
                    <strong style={{ color: "var(--text-primary)" }}>{advertiserStats.clicks}</strong>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "0.82rem" }}>
                    <span style={{ color: "var(--text-secondary)", display: "flex", alignItems: "center", gap: "4px" }}>
                      <Heart size={12} /> Favoritos:
                    </span>
                    <strong style={{ color: "var(--text-primary)" }}>{advertiserStats.favorites}</strong>
                  </div>
                </div>

                <div>
                  <div className="modal-info-label">Cidade / Bairro</div>
                  <div className="modal-info-value">{selectedAdvertiser.city || "-"} / {selectedAdvertiser.neighborhood || "-"}</div>

                  <div className="modal-info-label">Plano Ativo</div>
                  <div className="modal-info-value">
                    <span className={`badge ${(selectedAdvertiser.plan === 'monthly' || selectedAdvertiser.plan === 'annual') ? 'badge-premium' : 'badge-free'}`}>
                      {selectedAdvertiser.plan === "monthly" ? "Mensal (PREMIUM)" : selectedAdvertiser.plan === "annual" ? "Anual (PREMIUM)" : "Gratuito"}
                    </span>
                  </div>

                  <div className="modal-info-label">Vencimento do Plano</div>
                  <div className="modal-info-value">
                    {selectedAdvertiser.plan_expires_at ? formatDate(selectedAdvertiser.plan_expires_at).split(" ")[0] : "-"}
                  </div>
              ...
              <div className="modal-tab-container">
                <nav className="tab-nav">
                  <button 
                    ...
                    onClick={() => { setAdvertiserTab("info"); setIsEditing(false); }}
                  >
                    <Edit size={14} />
                    Informações & Edição
                  </button>
                ...
                <div className="tab-panel">
                  {modalLoading && (
                    <div style={{ textAlign: "center", padding: "2rem" }}>
                      <div className="spinner" style={{ margin: "0 auto 1rem", width: "24px", height: "24px" }}></div>
                      <p style={{ fontSize: "0.85rem" }}>Carregando dados do anunciante...</p>
                    </div>
                  )}
                  ...
                  {!modalLoading && advertiserTab === "info" && (
                    <>
                      {isEditing ? (
                        <form onSubmit={handleSaveAdvertiserInfo} className="edit-form-grid">
                          <div className="form-group">
                            <label className="form-label">Nome Fantasia / Profissional</label>
                            <input 
                              type="text" 
                              className="form-input" 
                          ...
                          <div className="form-group edit-form-full">
                            <label className="form-label">Descrição das Atividades / Sobre</label>
                            <textarea 
                              className="reason-textarea" 
                              style={{ height: "90px" }}
                          ...
                          <div className="form-group">
                            <label className="form-label">Expiração do Plano</label>
                            <input 
                              type="date" 
                              className="form-input" 
                              style={{ paddingLeft: "0.75rem" }} 
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                          <div>
                            <span className="modal-info-label">Descrição das Atividades</span>
                            <p className="modal-info-value" style={{ lineHeight: "1.5", fontSize: "0.9rem" }}>
                              {selectedAdvertiser.description || "Nenhuma descrição fornecida."}
                            </p>
                          </div>

                          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                            <div>
                              <span className="modal-info-label">Endereço Físico</span>
                              <p className="modal-info-value" style={{ display: "flex", alignItems: "flex-start", gap: "4px" }}>
                                <MapPin size={14} style={{ color: "var(--accent-orange)", flexShrink: 0, marginTop: "2px" }} />
                                <span>{selectedAdvertiser.address || "Sem endereço cadastrado"}</span>
                              </p>
                            </div>
                          ...
                          <div>
                            <span className="modal-info-label">Galeria de Fotos do Anúncio</span>
                            {selectedAdvertiser.gallery && selectedAdvertiser.gallery.length > 0 ? (
                              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", marginTop: "0.5rem" }}>
                                {selectedAdvertiser.gallery.map((imgUrl: string, idx: number) => (
                                  <img 
                                    key={idx} 
                                    src={imgUrl} 
                                    alt={`Foto ${idx + 1}`} 
                                    style={{ width: "70px", height: "70px", borderRadius: "6px", objectFit: "cover", border: "1px solid var(--border-color)" }}
                                  />
                                ))}
                              </div>
                            ) : (
                              <p style={{ color: "var(--text-muted)", fontSize: "0.85rem", display: "flex", alignItems: "center", gap: "4px", marginTop: "0.25rem" }}>
                                <ImageIcon size={14} /> Nenhuma imagem cadastrada na galeria.
                              </p>
                            )}
                          </div>

                          <div style={{ marginTop: "1rem", borderTop: "1px solid var(--border-color)", paddingTop: "1rem" }}>
                            <button className="btn btn-primary" style={{ width: "auto" }} onClick={() => setIsEditing(true)}>
                  ...
                  {!modalLoading && advertiserTab === "reviews" && (
                    <>
                      {advertiserReviews.length === 0 ? (
                        <p style={{ color: "var(--text-muted)", fontSize: "0.88rem" }}>Nenhuma avaliação recebida para este anunciante.</p>
                      ) : (
                        advertiserReviews.map((rev) => (
                          <div key={rev.id} className="review-item-card">
                            <div className="review-item-header">
                              <div className="user-cell">
                                {rev.user_avatar ? (
                                  <img src={rev.user_avatar} alt={rev.user_name} style={{ width: "24px", height: "24px", borderRadius: "50%" }} />
                                ) : (
                                  <div className="user-avatar-small" style={{ width: "24px", height: "24px", fontSize: "0.7rem", backgroundColor: "var(--accent-purple)" }}>
                              ...
                              <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                                {formatDate(rev.created_at || rev.createdAt).split(" ")[0]}
                              </span>
                            </div>
                            <div className="stars-row" style={{ marginBottom: "0.5rem" }}>
                              {Array.from({ length: 5 }).map((_, i) => (
                                <Star 
                                  key={i} 
                                  size={10} 
                              ...
                              <span style={{ fontSize: "0.75rem", color: "var(--text-primary)", marginLeft: "4px" }}>({rev.rating})</span>
                            </div>
                            <p style={{ fontSize: "0.85rem", color: "var(--text-secondary)", fontStyle: "italic" }}>
                              "{rev.comment}"
                            </p>
                  ...
                  {!modalLoading && advertiserTab === "payments" && (
                    <>
                      {advertiserPayments.length === 0 ? (
                        <p style={{ color: "var(--text-muted)", fontSize: "0.88rem" }}>Nenhum pagamento registrado para este anunciante.</p>
                      ) : (
                        <div className="table-wrapper">
                          <table>
                            <thead>
                            ...
                            <tbody>
                              {advertiserPayments.map((pay) => (
                                <tr key={pay.id}>
                                  ...
                                  <td>
                                    <span className="badge badge-premium">
                                      {pay.plano === "mensal" ? "Mensal" : "Anual"}
                                    </span>
                                  </td>
                                  <td style={{ fontWeight: 600 }}>R$ {pay.valor?.toFixed(2)}</td>
                                  <td>{formatDate(pay.data_pagamento).split(" ")[0]}</td>
                                  <td style={{ textTransform: "uppercase" }}>{pay.metodo}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                ...
                {!isEditing && (
                  <div className="action-card" style={{ backgroundColor: "rgba(37, 211, 102, 0.02)" }}>
                    <div className="action-card-header">
                    ...
                    <div className="btn-group-row">
                      ...
                      {selectedAdvertiser.is_active && (selectedAdvertiser.status || "ativo") === "ativo" && (
                        <button 
                          className="btn-action btn-action-suspend"
                          onClick={() => handleAdvertiserStatusAction("desativar")}
                          disabled={modalLoading}
                        >
                      ...
                      {(selectedAdvertiser.status || "ativo") !== "suspenso" && (
                        <button 
                          className="btn-action btn-action-block"
                          onClick={() => handleAdvertiserStatusAction("suspender")}
                          disabled={modalLoading}
                        >
            ...
            <div className="modal-footer">
              <button className="btn btn-logout" style={{ width: "auto" }} onClick={() => setSelectedAdvertiser(null)}>
                Fechar
              </button>
            </div>
          </div>
      ...
      {selectedReport && (
        <div className="modal-overlay" onClick={() => setSelectedReport(null)}>
          <div className="modal-content" style={{ maxWidth: "850px" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body" style={{ gridTemplateColumns: "1.2fr 1.8fr" }}>
              ...
              <div className="modal-sidebar-info">
                <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                  ...
                  <div>
                    <span className="modal-info-label">Status do Relatório</span>
                    <span 
                      className={`badge ${(selectedReport.status || "pendente") === "resolvido" ? "badge-active" : "badge-inactive"}`}
                      style={{
                        backgroundColor: (selectedReport.status || "pendente") === "pendente" ? "rgba(239, 68, 68, 0.15)" : undefined,
                        color: (selectedReport.status || "pendente") === "pendente" ? "#ef4444" : undefined,
                        borderColor: (selectedReport.status || "pendente") === "pendente" ? "rgba(239, 68, 68, 0.2)" : undefined
                      }}
                    >
                  ...
                  {selectedReport.details && (
                    <div>
                      <span className="modal-info-label">Comentários / Detalhes</span>
                      <div className="modal-info-value" style={{ 
                  ...
                  <div style={{ marginTop: "1rem", borderTop: "1px solid var(--border-color)", paddingTop: "1rem" }}>
                    <span className="modal-info-label" style={{ display: "flex", alignItems: "center", gap: "4px", marginBottom: "0.5rem" }}>
                      <History size={12} style={{ color: "var(--accent-blue)" }} />
                      Histórico da Ação
                    </span>
                    {reportActionsList.filter(a => a.report_id === selectedReport.id).length === 0 ? (
                      <p style={{ color: "var(--text-muted)", fontSize: "0.75rem", fontStyle: "italic" }}>
                        Nenhuma ação registrada para esta denúncia.
                      </p>
                    ) : (
                      reportActionsList.filter(a => a.report_id === selectedReport.id).map(action => (
                        <div 
                          key={action.id} 
                          style={{ 
                        ...
                        >
                          <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 600 }}>
                            <span style={{ textTransform: "uppercase", color: "var(--accent-blue)" }}>{action.action_type}</span>
                            <span style={{ color: "var(--text-muted)" }}>{formatDate(action.created_at).split(" ")[0]}</span>
                          </div>
                          <p style={{ color: "var(--text-secondary)" }}>{action.reason}</p>
                          <span style={{ color: "var(--text-muted)", fontSize: "0.68rem" }}>Responsável: {action.admin_email}</span>
                        </div>
                      ))
                    )}
                  </div>
              ...
              <div className="modal-tab-container" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                {/* 1. Autor (Reporter) Profile Summary */}
                <div className="action-card" style={{ backgroundColor: "rgba(59, 130, 246, 0.02)", margin: 0 }}>
                  <div className="action-card-header" style={{ borderBottom: "1px solid var(--border-color)", paddingBottom: "0.5rem", marginBottom: "0.5rem" }}>
                    <User size={14} style={{ color: "var(--accent-blue)" }} />
                    <span style={{ fontWeight: 600, fontSize: "0.85rem" }}>Denunciante (Autor)</span>
                  </div>
                  {(() => {
                    const reporter = usersList.find(u => u.open_id === selectedReport.reporter_id);
                    if (!reporter) return <p style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>Dados do autor indisponíveis</p>;
                    return (
                      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", fontSize: "0.8rem" }}>
                        <div>
                          <span style={{ color: "var(--text-secondary)" }}>Nome:</span>
                          <strong style={{ color: "var(--text-primary)", display: "block" }}>{reporter.name || "-"}</strong>
                        </div>
                        <div>
                          <span style={{ color: "var(--text-secondary)" }}>Email:</span>
                          <strong style={{ color: "var(--text-primary)", display: "block" }}>{reporter.email || "-"}</strong>
                        </div>
                        <div>
                          <span style={{ color: "var(--text-secondary)" }}>Telefone:</span>
                          <strong style={{ color: "var(--text-primary)", display: "block" }}>{getUserPhone(reporter)}</strong>
                        </div>
                        <div>
                          <span style={{ color: "var(--text-secondary)" }}>Data Cadastro:</span>
                          <strong style={{ color: "var(--text-primary)", display: "block" }}>{formatDate(reporter.created_at).split(" ")[0]}</strong>
                        </div>
                      </div>
                    );
                  })()}
                ...
                <div className="action-card" style={{ backgroundColor: "rgba(239, 68, 68, 0.02)", margin: 0 }}>
                  <div className="action-card-header" style={{ borderBottom: "1px solid var(--border-color)", paddingBottom: "0.5rem", marginBottom: "0.5rem" }}>
                    <ShieldAlert size={14} style={{ color: "var(--accent-orange)" }} />
                    <span style={{ fontWeight: 600, fontSize: "0.85rem" }}>Alvo Denunciado ({selectedReport.reported_type.toUpperCase()})</span>
                  </div>
                  {(() => {
                    const type = selectedReport.reported_type;
                    const reportedId = selectedReport.reported_id;
                    if (type === "cliente") {
                      const client = usersList.find(u => u.open_id === reportedId);
                      if (!client) return <p style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>Cliente não encontrado ou excluído</p>;
                      return (
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", fontSize: "0.8rem" }}>
                          <div>
                            <span style={{ color: "var(--text-secondary)" }}>Nome:</span>
                          ...
                          <div>
                            <span style={{ color: "var(--text-secondary)" }}>Email:</span>
                            <strong style={{ color: "var(--text-primary)", display: "block" }}>{client.email || "-"}</strong>
                          </div>
                          <div>
                            <span style={{ color: "var(--text-secondary)" }}>Status Conta:</span>
                            <strong style={{ color: "var(--text-primary)", display: "block" }}>
                              <span className={`badge ${(client.status || "ativo") === "ativo" ? "badge-active" : "badge-inactive"}`}>
                          ...
                          <div>
                            <span style={{ color: "var(--text-secondary)" }}>Telefone:</span>
                            <strong style={{ color: "var(--text-primary)", display: "block" }}>{getUserPhone(client)}</strong>
                          </div>
                        </div>
                      );
                    } else {
                      const provider = providersList.find(p => p.id === reportedId || p.user_id === reportedId);
                      if (!provider) return <p style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>Profissional/Estabelecimento não encontrado ou excluído</p>;
                      return (
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", fontSize: "0.8rem" }}>
                          <div>
                            <span style={{ color: "var(--text-secondary)" }}>Nome Negócio:</span>
                            <strong style={{ color: "var(--text-primary)", display: "block" }}>{provider.name}</strong>
                          </div>
                          <div>
                            <span style={{ color: "var(--text-secondary)" }}>Categoria:</span>
                            <strong style={{ color: "var(--text-primary)", display: "block" }}>{provider.category || "-"}</strong>
                          </div>
                          <div>
                            <span style={{ color: "var(--text-secondary)" }}>Cidade / Bairro:</span>
                            <strong style={{ color: "var(--text-primary)", display: "block" }}>{provider.city || "-"} / {provider.neighborhood || "-"}</strong>
                          </div>
                          <div>
                            <span style={{ color: "var(--text-secondary)" }}>Status Anúncio:</span>
                            <strong style={{ color: "var(--text-primary)", display: "block" }}>
                              <span className={`badge ${provider.is_active && (provider.status || "ativo") === "ativo" ? "badge-active" : "badge-inactive"}`}>
                                {provider.status === "suspenso" ? "SUSPENSO" : (provider.is_active ? "ATIVO" : "INATIVO")}
                              </span>
                            </strong>
                          </div>
                        </div>
                ...
                {selectedReport.status === "pendente" ? (
                  <div className="action-card" style={{ backgroundColor: "rgba(37, 211, 102, 0.02)", margin: 0, display: "flex", flexDirection: "column", gap: "0.75rem" }}>
                    <div className="action-card-header">
                      <UserCheck size={14} style={{ color: "var(--accent-primary)" }} />
                      <span style={{ fontWeight: 600, fontSize: "0.85rem" }}>Painel de Resolução Administrativa</span>
                    </div>

                    <div className="form-group" style={{ margin: 0 }}>
                      <label className="form-label" style={{ fontSize: "0.75rem" }}>Justificativa / Parecer do Administrador (Obrigatório)</label>
                      <textarea
                        className="reason-textarea"
                        ...
                        style={{ height: "65px", fontSize: "0.8rem", padding: "0.4rem" }}
                      />
                    </div>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                      <button
                        className="btn-action btn-action-reactivate"
                        style={{ flex: 1, minWidth: "120px", fontSize: "0.75rem" }}
                  ...
                  <div className="action-card" style={{ backgroundColor: "rgba(37, 211, 102, 0.05)", margin: 0, textAlign: "center", padding: "1rem" }}>
                    <span style={{ color: "var(--accent-primary)", fontWeight: 600, fontSize: "0.9rem", display: "flex", alignItems: "center", justifyContent: "center", gap: "4px" }}>
                      <UserCheck size={16} /> Esta denúncia foi resolvida
                    </span>
                    <p style={{ color: "var(--text-secondary)", fontSize: "0.8rem", marginTop: "0.25rem" }}>
                      Consulte o Histórico de Ações ao lado para ver a justificativa e o responsável.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="modal-footer">
              <button className="btn btn-logout" style={{ width: "auto" }} onClick={() => setSelectedReport(null)}>
                Fechar
              </button>
            </div>
          </div>
