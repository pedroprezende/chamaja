  ...
  const reverseGeocode = async (lat: number, lon: number): Promise<string> => {
    try {
      ...
      if (data && data.address) {
        const { road, suburb, house_number, city, town, village } =
          data.address;
        ...
        return (
          parts.join(", ") || data.display_name || "Minha Localização (GPS)"
        );
      }
      ...
      if (Location) {
        ...
        if (status === "granted") {
          ...
          try {
            ...
            console.warn(
              "getLastKnownPositionAsync failed, continuing to current position query",
              lkErr,
            );
          }
          ...
          try {
            ...
            console.warn(
              "getCurrentPositionAsync failed or timed out:",
              currErr,
            );
