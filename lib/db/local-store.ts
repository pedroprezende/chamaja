...
const SEED_SUB_SERVICES: DbSubService[] = [
  { id: "eletricista", category_id: "reformas-reparos", name: "Eletricista", icon: "electrical-services", image_url: "https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=400&q=80", is_active: true, display_order: 1, created_at: now(), updated_at: now() },
...
const SEED_REGIONS: DbRegion[] = [
  { id: "braganca-paulista", name: "Bragança Paulista", state: "SP", providers_count: 128, ads_count: 12, is_active: true, created_at: now(), updated_at: now() },
  { id: "atibaia", name: "Atibaia", state: "SP", providers_count: 98, ads_count: 8, is_active: true, created_at: now(), updated_at: now() },
  { id: "extrema", name: "Extrema", state: "MG", providers_count: 45, ads_count: 4, is_active: true, created_at: now(), updated_at: now() },
  { id: "itatiba", name: "Itatiba", state: "SP", providers_count: 62, ads_count: 6, is_active: true, created_at: now(), updated_at: now() },
  { id: "camanducaia", name: "Camanducaia", state: "MG", providers_count: 28, ads_count: 3, is_active: false, created_at: now(), updated_at: now() },
  { id: "piracaia", name: "Piracaia", state: "SP", providers_count: 33, ads_count: 2, is_active: true, created_at: now(), updated_at: now() },
];

const SEED_FEATURED_ADS: DbFeaturedAd[] = [
  { id: "feat-1", provider_id: "p1", provider_name: "Elétrica do Zé", provider_avatar: "https://images.unsplash.com/photo-1621905251189-08b45d6a269e?w=80&q=70", category_name: "Eletricista", views: 1256, is_featured: true, display_order: 1, created_at: now(), updated_at: now() },
  { id: "feat-2", provider_id: "p2", provider_name: "Marmitaria Fit", provider_avatar: "https://images.unsplash.com/photo-1490645935967-10de6ba17061?w=80&q=70", category_name: "Alimentação", views: 965, is_featured: true, display_order: 2, created_at: now(), updated_at: now() },
  { id: "feat-3", provider_id: "p3", provider_name: "Top Barber", provider_avatar: "https://images.unsplash.com/photo-1503951914875-452162b0f3f1?w=80&q=70", category_name: "Barbearia", views: 789, is_featured: true, display_order: 3, created_at: now(), updated_at: now() },
  { id: "feat-4", provider_id: "p4", provider_name: "Bragança Limpeza", provider_avatar: "https://images.unsplash.com/photo-1558618047-3c8c76ca7d13?w=80&q=70", category_name: "Limpeza", views: 650, is_featured: true, display_order: 4, created_at: now(), updated_at: now() },
  { id: "feat-5", provider_id: "p5", provider_name: "Studio Ink Tattoo", provider_avatar: "https://images.unsplash.com/photo-1565193566173-7a0ee3dbe261?w=80&q=70", category_name: "Tatuador", views: 542, is_featured: true, display_order: 5, created_at: now(), updated_at: now() },
  { id: "feat-6", provider_id: "p6", provider_name: "Encanador Rápido", provider_avatar: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=80&q=70", category_name: "Encanador", views: 430, is_featured: false, display_order: 6, created_at: now(), updated_at: now() },
];

// ── Inicialização (seed apenas se vazio) ──────────────────────────────────────
async function seedIfEmpty<T>(table: keyof typeof KEYS, seed: T[]): Promise<void> {
  try {
    const raw = await AsyncStorage.getItem(KEYS[table]);
    if (raw === null) {
...
export const categoriesDB = {
  async list(): Promise<DbListResult<DbCategory>> {
    try {
      ...
      return { data: data.sort((a, b) => a.display_order - b.display_order), error: null };
    } catch (e: any) {
      return { data: [], error: e.message };
    }
  },

  async insert(input: { name: string; icon: string }): Promise<DbResult<DbCategory>> {
    try {
  ...
  async update(id: string, patch: Partial<Omit<DbCategory, "id" | "created_at">>): Promise<DbResult<DbCategory>> {
    try {
      const rows = await load<DbCategory>("categories");
  ...
  async delete(id: string): Promise<DbResult<boolean>> {
    try {
      ...
      await save("sub_services", subs.filter((s) => s.category_id !== id));
  ...
  async reorder(ids: string[]): Promise<DbResult<boolean>> {
    try {
      ...
      const updated = rows.map((r) => {
        ...
        return { ...r, display_order: idx !== -1 ? idx + 1 : r.display_order, updated_at: now() };
      });
...
export const subServicesDB = {
  async listByCategory(categoryId: string): Promise<DbListResult<DbSubService>> {
    try {
  ...
  async insert(input: { category_id: string; name: string; icon?: string; image_url?: string }): Promise<DbResult<DbSubService>> {
    try {
      const rows = await load<DbSubService>("sub_services");
  ...
  async update(id: string, patch: Partial<Omit<DbSubService, "id" | "created_at">>): Promise<DbResult<DbSubService>> {
    try {
      const rows = await load<DbSubService>("sub_services");
      const idx = rows.findIndex((r) => r.id === id);
      if (idx === -1) return { data: null, error: "Serviço não encontrado" };
      const updated: DbSubService = { ...rows[idx], ...patch, updated_at: now() };
      const next = [...rows];
      next[idx] = updated;
  ...
  async delete(id: string): Promise<DbResult<boolean>> {
    try {
      const rows = await load<DbSubService>("sub_services");
      await save("sub_services", rows.filter((r) => r.id !== id));
      return { data: true, error: null };
    } catch (e: any) {
      return { data: null, error: e.message };
    }
...
export const regionsDB = {
  ...
  async insert(input: { name: string; state: string }): Promise<DbResult<DbRegion>> {
    try {
  ...
  async update(id: string, patch: Partial<Omit<DbRegion, "id" | "created_at">>): Promise<DbResult<DbRegion>> {
    try {
      const rows = await load<DbRegion>("regions");
      const idx = rows.findIndex((r) => r.id === id);
  ...
  async delete(id: string): Promise<DbResult<boolean>> {
    try {
      const rows = await load<DbRegion>("regions");
      await save("regions", rows.filter((r) => r.id !== id));
      return { data: true, error: null };
    } catch (e: any) {
      return { data: null, error: e.message };
    }
...
export const featuredAdsDB = {
  ...
  async toggleFeatured(id: string): Promise<DbResult<DbFeaturedAd>> {
    try {
      ...
      const updated = { ...rows[idx], is_featured: !rows[idx].is_featured, updated_at: now() };
      const next = [...rows];
      next[idx] = updated;
  ...
  async updateOrder(id: string, display_order: number): Promise<DbResult<boolean>> {
    try {
      const rows = await load<DbFeaturedAd>("featured_ads");
