...
export default function DashboardAdmin() {
  ...
  const { data: dashboardData, isLoading } =
    trpc.dashboard.getAdminStats.useQuery(undefined, {
      enabled: isAdmin,
    });
  ...
  const { chartPoints, chartLinePath, chartFillPath, chartGridPoints } =
    useMemo(() => {
      ...
      const points = chartData.map((d, index) => {
        ...
        const x =
          chartPadding + (index * (chartWidth - 2 * chartPadding - 10)) / 5;
        // Scale Y inversely (higher value = lower Y coordinate in SVG space)
        const y =
          chartHeight -
          chartPadding -
          (d.revenue / maxVal) * (chartHeight - 2 * chartPadding);
      ...
      return {
        ...
        chartFillPath: fillPath,
        chartGridPoints: gridPoints,
      };
    ...
    <View style={styles.screen}>
      ...
      >
        ...
        >
          ...
          <MaterialIcons
            name={
              showRegionPicker ? "keyboard-arrow-up" : "keyboard-arrow-down"
            }
            size={18}
            color="#6B7280"
          />
          ...
          <>
            ...
            >
              {[
                ...
                {
                  label: "Cliques no\nWhatsApp",
                  value: dashboardData?.stats.whatsappClicks || 0,
                  sub: "Hoje",
                  icon: "chat",
                  color: "#F0FDF4",
                  iconColor: "#16A34A",
                ...
                {
                  label: "Visualizações\nde serviços",
                  value: dashboardData?.stats.serviceViews || 0,
                  sub: "Hoje",
                  icon: "bar-chart",
                  color: "#FFF7ED",
                  iconColor: "#EA580C",
                ...
                <Pressable
                  ...
                  style={({ pressed }) => [
                    ...
                    pressed && stat.onPress && { opacity: 0.85 },
                  ]}
            ...
            >
              <View style={styles.mrrHeader}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.mrrLabel}>
                    Receita Bruta Mensalizada (MRR)
                  </Text>
                  <Text style={styles.mrrValue}>
                    R${" "}
                    {dashboardData?.stats.monthlyRecurringRevenue
                      ?.toFixed(2)
                      .replace(".", ",") || "0,00"}
                  </Text>
                </View>
            ...
            <View style={styles.topServiceCard}>
              <View style={styles.topServiceIcon}>
                <MaterialIcons
                  name={
                    (dashboardData?.topService.icon as any) ||
                    "electrical-services"
                  }
                  size={26}
                  color="#25D366"
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.topServiceName}>
                  {dashboardData?.topService.name || "-"}
                </Text>
                <Text style={styles.topServicePct}>
                  {dashboardData?.topService.percentage || 0}% das buscas de
                  hoje
                </Text>
              </View>
            ...
            <View style={styles.activityList}>
              {(dashboardData?.recentSearches || []).map((item, i) => {
                ...
                const isSuggestion =
                  item.query.startsWith("[sugestão]:") ||
                  item.query.startsWith("[SUGESTÃO]:");
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.activityItem,
                      i < (dashboardData?.recentSearches?.length || 0) - 1 &&
                  ...
                  >
                    <View
                      style={[
                        ...
                        {
                          backgroundColor: isSuggestion ? "#FEF3C7" : "#E0F2FE",
                        },
                      ]}
                    >
                      <MaterialIcons
              ...
              {(!dashboardData?.recentSearches ||
                ...
                <View
                  style={[styles.activityItem, { justifyContent: "center" }]}
                >
                  <Text style={{ color: "#6B7280" }}>
                    Nenhuma busca recente registrada.
                  </Text>
                </View>
              )}
            </View>
            ...
            <View style={styles.activityHeader}>
              ...
              <Pressable
                onPress={() => router.push("/admin/prestadores-admin" as any)}
              >
                <Text style={styles.verTudo}>Ver prestadores</Text>
              </Pressable>
            </View>
            ...
            <View style={styles.activityList}>
              {(dashboardData?.recentActivity || []).map((item, i) => (
                <View
                  ...
                  style={[
                    styles.activityItem,
                    i < (dashboardData?.recentActivity?.length || 0) - 1 &&
                      styles.activityBorder,
                  ]}
                >
