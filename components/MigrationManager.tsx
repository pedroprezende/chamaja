...
export function MigrationManager() {
  ...
  // Queries
  const { data: dbServices = [], isLoading: loadingS } =
    trpc.services.list.useQuery();
  const { data: dbAds = [], isLoading: loadingA } =
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      ...
      const localAds = await adsDB.getAll();
      if (
        (dbServices.length === 0 && localServices.length > 0) ||
        (dbAds.length === 0 && localAds.length > 0)
      ) {
        setShowManual(true);
      }
    }, 2000);
  ...
  if (migrating) {
    return (
      <View style={styles.container}>
        ...
        <Text
          style={[styles.text, status.includes("Erro") && { color: "#DC2626" }]}
        >
          {status}
        </Text>
      </View>
