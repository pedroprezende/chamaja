...
const mockSubs = [
  {
    id: "reformas-reparos-eletricista",
    categoryId: "reformas-reparos",
    name: "Eletricista",
  },
  {
    id: "reformas-reparos-encanador",
    categoryId: "reformas-reparos",
    name: "Encanador",
  },
  {
    id: "reformas-reparos-pedreiro",
    categoryId: "reformas-reparos",
    name: "Pedreiro",
  },
  {
    id: "reformas-reparos-pintor",
    categoryId: "reformas-reparos",
    name: "Pintor",
  },
  {
    id: "reformas-reparos-chaveiro",
    categoryId: "reformas-reparos",
    name: "Chaveiro",
  },
  {
    id: "servicos-domesticos-diarista",
    categoryId: "servicos-domesticos",
    name: "Diarista",
  },
  {
    id: "servicos-domesticos-faxineira",
    categoryId: "servicos-domesticos",
    name: "Faxineira",
  },
  {
    id: "servicos-externos-jardineiro",
...
function runTest() {
  ...
  for (const phrase of testPhrases) {
    ...
    for (const [kw, mapInfo] of Object.entries(keywordMap)) {
      ...
      if (queryNorm.includes(kwNorm)) {
        ...
        if (mapInfo.categoryId) {
          const matchedSub = mockSubs.find(
            (s) =>
          ...
          if (matchedSub) {
            ...
            matchScores[key] = {
              ...
              name:
                mockCats.find((c) => c.id === mapInfo.categoryId)?.name ||
                mapInfo.categoryId,
              score: (matchScores[key]?.score || 0) + points,
            };
          }
    ...
    if (bestMatch && bestMatch.score >= THRESHOLD) {
      ...
      console.log(
        ` -> Mapeado para: [${bestMatch.type.toUpperCase()}] "${bestMatch.name}" (Score: ${bestMatch.score})`,
      );
    } else {
      console.log(`Frase: "${phrase}"`);
      console.log(
        ` -> Ambiguidade detectada (Confidence baixa). Abriria o assistente Xará.`,
      );
    }
