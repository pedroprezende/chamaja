...
const testPhrases = [
  ...
  "conserto de tela de celular samsung",
  "corte de cabelo masculino ou barba",
  "limpeza e cloro de piscina",
  "preciso de um mecanico para consertar meu carro",
  "uma coisa aleatoria que nao deve mapear para nada",
];

async function test() {
  console.log("=== Testando Algoritmo de Busca Inteligente ===");
  const connection = await db.getDb();
  if (!connection) {
    console.log("Erro: Banco de dados indisponível.");
    return;
  }
  ...
  for (const phrase of testPhrases) {
    ...
    if (bestMatch && bestMatch.score >= THRESHOLD) {
      ...
      console.log(
        ` -> Sucesso! Mapeado para: [${bestMatch.type.toUpperCase()}] "${bestMatch.name}" (Score: ${bestMatch.score})`,
      );
    } else {
      console.log(`Frase: "${phrase}"`);
      console.log(
